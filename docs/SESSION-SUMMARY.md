...
1. **Project Architecture Defined**
   ...
   - JSON-RPC protocol over HTTP for plugin ↔ sidecar communication
   - Clear separation: Plugin (Java 21) + Sidecar (Go 1.22+)
...
3. **Project Structure Created**
   ...
   ├── copilot-bridge/       ✅ Go sidecar with HTTP server
   ├── integration-tests/    ✅ Test module structure
   └── docs/                 ✅ Comprehensive documentation
   ```

4. **Go Sidecar Implementation**
...
- [ ] `AgenticCopilotToolWindow.java` with 5 tabs (Prompt, Context, Plans, Timeline, Settings)
- [ ] `AgenticCopilotService.java` application service
- [ ] Register in `plugin.xml`
- [ ] Test in sandbox IDE

### 4. Sidecar Lifecycle Management (2-3 hours)
- [ ] `SidecarProcess.java` - Process management
- [ ] `SidecarClient.java` - HTTP JSON-RPC client
- [ ] `SidecarService.java` - Lifecycle orchestration
...
### Sidecar
- [x] Go binary structure complete
- [ ] Binary builds successfully
- [ ] Sidecar starts automatically
- [ ] Health check passes
- [ ] Sidecar stops cleanly

### Communication
- [x] JSON-RPC protocol defined
- [ ] Plugin can call sidecar `/health`
...
4. **Testing Priority**: Integration tests or unit tests first?
   ...
   - Integration test once sidecar lifecycle works
...
- `copilot-bridge/cmd/sidecar/main.go` - Sidecar entry point
...
- Test sidecar independently before integration
