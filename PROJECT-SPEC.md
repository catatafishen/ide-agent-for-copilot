# Agentic GitHub Copilot for JetBrains — Project Specification
...
- **IntelliJ Platform**: target **2025.1–2025.3**; set `sinceBuild`/`untilBuild` per platform timeline (e.g., `251` →
  `253.*`).
- **Runtime & language**:
...
- **60 tools** registered via MCP stdio server, executed through PSI bridge HTTP server inside IntelliJ process.
- **Categories**: Code navigation (6), File operations (7), Code quality & analysis (11), Refactoring (4), Testing (4),
  Project & build (8), Git (10), Infrastructure (5), Terminal (3), Editor (4).
- All file writes use IntelliJ Document API with proper undo/redo support.
...
intellij-copilot-plugin/
  ...
  plugin-core/
    ...
    src/test/java/...
  mcp-server/
    src/main/java/...       # MCP stdio server (Java 21)
...
- **Run plan**: prompt → choose model → stream plan/timeline → approve/deny tool calls → commit with Conventional Commit
  format.
...
- **Footprint**: lazy‑start; shutdown on IDE exit.
...
All plugin code is **Java 21**. If a Kotlin‑only API is unavoidable, we add a **thin Kotlin shim** behind a Java
...
See README.md for detailed implementation decisions on:

- Build system (Gradle with IntelliJ Platform Plugin)
- Protocol (ACP via Copilot CLI, MCP via stdio server)
- Authentication (GitHub OAuth via Copilot CLI)
- Development strategy (sandbox-first with fast reload)

---

*For agent behavior guidelines, see `copilot-instructions.md`*
