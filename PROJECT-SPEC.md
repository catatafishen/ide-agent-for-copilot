...
- **54 tools** registered via MCP stdio server, executed through PSI bridge HTTP server inside IntelliJ process.
- **Categories**: Code intelligence (12), File operations (8), Project management (6), Inspections & refactoring (10), Git (6), Testing (6), Others (6).
...
- **Run plan**: prompt → choose model → stream plan/timeline → approve/deny tool calls → commit with Conventional Commit format.
...
All plugin code is **Java 21**. If a Kotlin‑only API is unavoidable, we add a **thin Kotlin shim** behind a Java interface to keep the codebase Java‑centric.
